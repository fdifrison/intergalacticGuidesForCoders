

simple_client_browser_server

    Tutorial based on freeCodeCamp

    https://www.youtube.com/watch?v=o0XbHvKxw7Y&list=WL&index=3&t=445s

    github: https://github.com/csev/dj4e-samples

    www.dj4e.com

    Topic:

    - sockets, client, sever, browser


inventory_manager_system

    Tutorial based on CodingEntrepeneurs

    https://www.youtube.com/playlist?list=PLEsfXFp6DpzRMby_cSoWTFw8zaMdTEXgL

    github : https://github.com/codingforentrepreneurs/Try-Django-3.2

    Topic:

    -